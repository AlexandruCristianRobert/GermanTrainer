//
// Schreiben Teil 2 — the Nachricht KI-Tipp. One single-shot generateContent
// call, never a chat-role API (see useSprechenPartner.ts's architecture note —
// same reasoning applies here). Mirrors useSchreibenTipp.ts.
//
...
// point-covered, so the model JUDGES coverage itself from the Nachricht text;
...
//
// The one thing this tip knows that Teil 1's does not: the ANLASS. A Nachricht
// owes a specific communicative MOVE, not just content — an Entschuldigung that
// never offers a replacement is incomplete even when every Inhaltspunkt has
// been touched. Passing `ANLASS_LABEL[…].de` lets the tip name that debt
// („Die Entschuldigung steht — es fehlt noch ein konkreter Ersatzvorschlag").
//
...
import type { SchreibenNachricht } from '../data/schreibenNachricht'
...
/** Plan keywords not yet found (normalized substring) in the Nachricht text. */
function notYetWrittenKeywords(n: SchreibenNachricht): string[] {
  const hay = normalize(n.textDe)
  return n.plan
...
export const NACHRICHT_KITIPP_SCHEMA = {
  type: 'object',
...
export function buildNachrichtKiTippPrompt(n: SchreibenNachricht): string {
  const notYetSaid = notYetWrittenKeywords(n)
  ...
  const inhaltspunkteListe = n.auftrag.inhaltspunkte
    .map((p, i) => `${i}. ${p}`)
    .join('\n')
  const textSoFar = n.textDe.slice(-1200)

  return (
    `Aufgabe der Nachricht: ${n.auftrag.taskDe}\n` +
    `Anlass der Nachricht: ${ANLASS_LABEL[n.auftrag.anlass].de}\n` +
    `Die vier Inhaltspunkte des Aufgabenblatts:\n${inhaltspunkteListe}\n` +
    'Beurteile selbst anhand des NACHRICHT-BISHER-Texts unten, welche dieser ' +
    `Inhaltspunkte inhaltlich schon abgedeckt sind und welche noch offen sind. ${stichwortHinweisDe}\n\n` +
    `NACHRICHT BISHER (letzte 1200 Zeichen):\n${textSoFar}\n\n` +
    'Gib in 1–2 Sätzen (Deutsch, du-Form) einen strategischen Tipp, WELCHEN ' +
    'kommunikativen Schritt die Nachricht dem Anlass noch schuldet — z. B. „Die ' +
    'Entschuldigung steht — es fehlt noch ein konkreter Ersatzvorschlag" —, oder ' +
    'welchen noch offenen Inhaltspunkt der Lernende als Nächstes angehen könnte. ' +
    'Formuliere KEINEN fertigen Satz zum Abschreiben, nur die Richtung. Antworte ' +
    'ausschließlich als JSON-Objekt mit genau einem Feld "tippDe", z. B. ' +
    '{"tippDe": "Geh jetzt auf …"} — keine Markdown-Fences.'
  )
}

export async function generateNachrichtKiTipp(
  client: GeminiClient,
  model: string,
  ...
  const response = await client.models.generateContent({
    model,
    contents: buildNachrichtKiTippPrompt(n),
    config: {
      responseMimeType: 'application/json',
      responseSchema: NACHRICHT_KITIPP_SCHEMA as unknown as Record<string, unknown>,
      temperature: 0.7,
      topP: 0.95
    }
  })
