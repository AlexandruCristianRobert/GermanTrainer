...
import { buildDwSpecs } from '../../composables/useDwSentenceQuiz'

const STORAGE_KEY = 'dwSentenceSetup'
...
watch([levels, pairs, groups, nounsPer, wordHints, count, customCount], saveStored, { deep: true })
...
async function start() {
  ...
  sessionStorage.setItem(STASH_KEY, JSON.stringify({
    ...
    hints: wordHints.value
  }))
  router.push({ name: 'directionwords-sentence-run' })
}
...
<template>
  <div class="page setup-page">
    <header class="section-header">
      <div>
        <div class="breadcrumb">Modul · hin &amp; her · Satzübersetzung · Einrichtung</div>
        <h1 class="section-title">Setup<em>.</em></h1>
        <p class="section-subtitle">
          Pick a pair pool and a noun theme. The AI writes one English sentence per item that spells
          out the speaker's position in words — a scene, not a coin flip — so hin- or her- is genuinely
          determined by what you read. You translate into German; the AI grades your compound's side.
        </p>
      </div>
    </header>
    ...
    <div class="field">
      ...
      <p class="micro-mark grading-hint">Hints reveal nouns and unknown words — never the direction word; that one is yours.</p>
    </div>
    ...
    <div class="alert alert-info">
      ...
      We sample {{ effective }} sentence spec{{ effective === 1 ? '' : 's' }} — each one a
      drilled pair-element with a random hin/her side, plus 1–2 theme nouns — then the AI writes them one batch at
      a time. Each sentence spells out the speaker's position, so which side is correct is never a guess.
