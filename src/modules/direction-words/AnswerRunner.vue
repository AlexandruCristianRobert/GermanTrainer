...
import {
  dwLevelLabel, generateDwQuestionBatch, gradeDwReply, buildDwAnswerItem,
  type GeneratedDwQuestion, type DwSentenceSpec
} from '../../composables/useDwAnswerQuiz'
...
/** Per-card grade, kept local — mirrors DacAnswerVerdict in the da-compound
 *  AnswerRunner. No "correction" field: the exampleAnswer is always shown as
 *  one possible answer ("So könnte es klingen"), never as THE only correct
 *  answer — a direction word fronted or mid-field is accepted equally. */
interface DwAnswerVerdict {
  index: number
  correct: boolean
  tip?: string
  tags?: DwErrorTag[]
...
const deck = ref<GeneratedDwQuestion[]>([])             // arrival order
const generationDone = ref(false)
const level = ref('A2–C1')
const metaInfo = ref<Stash>({ specs: [] })

const answers = ref<string[]>([])
const verdicts = ref<Map<number, DwAnswerVerdict>>(new Map())
...
const current = computed<GeneratedDwQuestion | null>(() => deck.value[index.value] ?? null)
...
onMounted(async () => {
  ...
  generateProgressively<DwSentenceSpec, GeneratedDwQuestion>({
    batches,
    runBatch: async (batch) => {
      const res = await generateDwQuestionBatch(client, { model: settings.value.model, specs: batch, level: level.value, maxRetries: 1 })
...
async function submit() {
  ...
  let verdict: DwAnswerVerdict
  try {
    const grade = await gradeDwReply(resolveAiClient(settings.value), {
...
function finishQuiz() {
  ...
  // Single direction — items always recorded (unlike dw-sentence, which
  // records EN→DE only; here there is only one direction to begin with).
  const items = deck.value.map((q, i) => buildDwAnswerItem(q, verdicts.value.get(i)?.correct ?? false, verdicts.value.get(i)?.tags))
  saveQuizRun({
    type: 'dw-answer',
    ...
    meta: {
      dwAnswerLevels: metaInfo.value.levels,
      dwAnswerPairs: metaInfo.value.pairs,
      dwAnswerGroups: metaInfo.value.groups,
      dwAnswerItems: items
    }
  })
}
...
// AI-family precedent (dac AnswerRunner/dw SentenceRunner): retrying the
// wrong items resets historySaved so finishQuiz() records a SECOND 'dw-answer'
...
function newQuiz() { router.push({ name: 'directionwords-answer' }) }
...
<template>
  ...
  <div v-else-if="finished" class="page result-page">
    <header class="section-header">
      <div>
        <div class="breadcrumb">Modul · hin &amp; her · Antworten · Auswertung</div>
    ...
    <div class="result-rows">
      <div v-for="(q, i) in deck" :key="i" class="ai-result-row" :class="{ good: verdicts.get(i)?.correct, bad: !verdicts.get(i)?.correct }">
        <div class="rr-head">
