...
import {
  dwLevelLabel, generateDwSentenceBatch, buildDwHintInputs, gradeDwAnswer, buildDwDrillItem,
  type GeneratedDwSentence, type DwSentenceSpec
} from '../../composables/useDwSentenceQuiz'
...
import { saveQuizRun, type DwErrorTag } from '../../composables/useQuizHistory'
...
/** Per-card grade, kept local — useDwSentenceQuiz has no exported Verdict shape
 *  (mirrors DacSentenceVerdict in the da-compound SentenceRunner). */
interface DwSentenceVerdict {
  index: number
  correct: boolean
...
const deck = ref<GeneratedDwSentence[]>([])             // arrival order
const generationDone = ref(false)
const level = ref('A2–C1')
const wordHints = ref(true)
const metaInfo = ref<Stash>({ specs: [] })

const answers = ref<string[]>([])
const verdicts = ref<Map<number, DwSentenceVerdict>>(new Map())
...
const current = computed<GeneratedDwSentence | null>(() => deck.value[index.value] ?? null)
...
// Per design decision 1 (useDwSentenceQuiz): hints never reveal the direction
// word — buildDwHintInputs already filters out anything that would leak the
// target compound or its vertical twin, so this just wires the segments up.
const currentSegments = computed<HintSegment[]>(() => {
  const s = current.value
  if (!s) return []
  if (!wordHints.value) return [{ text: s.english }]
  return buildHintSegments(s.english, buildDwHintInputs(s))
})
...
onMounted(async () => {
  ...
  level.value = dwLevelLabel(stash.levels ?? [])
  wordHints.value = stash.hints !== false
  ...
  generateProgressively<DwSentenceSpec, GeneratedDwSentence>({
    batches,
    runBatch: async (batch) => {
      const res = await generateDwSentenceBatch(client, { model: settings.value.model, specs: batch, level: level.value, maxRetries: 1 })
...
async function submit() {
  ...
  let verdict: DwSentenceVerdict
  try {
    const grade = await gradeDwAnswer(resolveAiClient(settings.value), {
...
function finishQuiz() {
  ...
  const items = deck.value.map((s, i) => buildDwDrillItem(s, verdicts.value.get(i)?.correct ?? false, verdicts.value.get(i)?.tags))
  saveQuizRun({
    type: 'dw-sentence',
    ...
    meta: {
      dwSentenceLevels: metaInfo.value.levels,
      dwSentencePairs: metaInfo.value.pairs,
      dwSentenceGroups: metaInfo.value.groups,
      dwSentenceNounsPer: metaInfo.value.nounsPer,
      dwSentenceHints: wordHints.value,
      dwSentenceItems: items
    }
  })
}
...
// AI-family precedent (dac SentenceRunner/VerbSentenceRunner): retrying the
// wrong items resets historySaved so finishQuiz() records a SECOND 'dw-sentence'
// history entry for this retry pass — deliberately different from the plain
// direction-word drills (T1-T5), which don't re-record retries. This keeps
...
function newQuiz() { router.push({ name: 'directionwords-sentence' }) }
function endQuiz() { router.push({ name: 'directionwords' }) }
...
<template>
  ...
  <div v-else-if="finished" class="page result-page">
    <header class="section-header">
      <div>
        <div class="breadcrumb">Modul · hin &amp; her · Satzübersetzung · Auswertung</div>
    ...
    <div class="setup-actions">
      <button class="btn btn-ghost" type="button" @click="endQuiz">← Direction Words</button>
