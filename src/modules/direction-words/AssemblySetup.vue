...
import { filterDwAssemblyItems } from '../../composables/useDirectionAssemblyQuiz'
import { DIRECTION_LEVELS, type DirectionLevel } from '../../data/directionWords'

const STORAGE_KEY = 'dwAssemblySetup'
...
const availableItems = computed(() => filterDwAssemblyItems({ levels: levels.value }).length)
...
function start() {
  router.push({
    name: 'directionwords-assembly-run',
    query: {
...
<template>
  <div class="page setup-page">
    <header class="section-header">
      <div>
        <div class="breadcrumb">Modul · hin &amp; her · Satzbau</div>
        ...
        <p class="section-subtitle">
          ...
          Every tile arrives pre-inflected, so only word order counts: the direction
          word lands at the true clause end, or fuses onto the verb when German
          demands it. Some sentences accept a curated fronted variant too.
        </p>
      </div>
    </header>
