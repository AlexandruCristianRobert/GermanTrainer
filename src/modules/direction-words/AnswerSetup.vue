...
import { buildDwSpecs } from '../../composables/useDwAnswerQuiz'

const STORAGE_KEY = 'dwAnswerSetup'
...
interface Stored {
  levels?: DirectionLevel[]; pairs?: string[]; groups?: NounGroup[]
...
function saveStored(): void {
  try {
    localStorage.setItem(STORAGE_KEY, JSON.stringify({
      levels: [...levels.value], pairs: [...pairs.value], groups: [...groups.value],
...
onMounted(async () => {
  ...
  const s = loadStored()
  if (s) {
    if (Array.isArray(s.levels)) levels.value = s.levels.filter(l => (DIRECTION_LEVELS as readonly string[]).includes(l))
    if (Array.isArray(s.pairs)) pairs.value = s.pairs.filter(p => PAIR_ELEMENTS.includes(p))
...
watch([levels, pairs, groups, nounsPer, count, customCount], saveStored, { deep: true })
...
const canStart = computed(() =>
  canUseAi.value && levels.value.length > 0 && pairs.value.length > 0 && selectedNounTotal.value > 0
)
...
async function start() {
  ...
  const specs = buildDwSpecs(pairs.value, nounPool, n, nounsPer.value)
  ...
  router.push({ name: 'directionwords-answer-run' })
}

function back() { router.push({ name: 'directionwords' }) }
</script>
...
<template>
  <div class="page setup-page">
    <header class="section-header">
      <div>
        <div class="breadcrumb">Modul · hin &amp; her · Antworten · Einrichtung</div>
        <h1 class="section-title">Setup<em>.</em></h1>
        <p class="section-subtitle">
          Pick a pair pool and a noun theme. The AI sets the scene in German and asks one natural question per
          item — your position is always spelled out, so hin- or her- is genuinely determined by what you read.
          You answer in German; the grader judges whether it actually answers the question and whether the
          direction word (and the rest of the grammar) is right.
        </p>
      </div>
    </header>
    ...
    <div class="field">
      ...
      <div class="chip-row">
        <button v-for="l in DIRECTION_LEVELS" :key="l" class="chip" :class="{ selected: levels.includes(l) }" @click="levels = toggle(levels, l)">{{ l }}</button>
      </div>
    </div>
    ...
    <div class="field">
      ...
      <div class="chip-row">
        <button v-for="p in ADVERB_PAIRS" :key="p.element" class="chip" :class="{ selected: pairs.includes(p.element) }" @click="pairs = toggle(pairs, p.element)">{{ pairLabel(p.element) }}</button>
      </div>
    </div>
    ...
    <div v-if="pairs.length === 0" class="alert alert-warning"><span class="alert-label">Warning</span>Select at least one pair.</div>
    ...
    <div class="alert alert-info">
      ...
      We sample {{ effective }} question spec{{ effective === 1 ? '' : 's' }} — each one a drilled pair-element
      with a random hin/her side, plus 1–2 theme nouns — then the AI writes one natural German scenario question
      per item, one batch at a time. The quiz opens on the first question and the rest stream in as you go. Type
      your answer in German; a correct answer may put the direction word in the Mittelfeld or fronted — both
      accepted equally, as long as it actually answers the question and the grammar holds up.
    </div>
