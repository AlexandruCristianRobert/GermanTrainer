...
import { filterHinHerItems } from '../../composables/useDirectionDrill'
import { DIRECTION_LEVELS, type DirectionLevel } from '../../data/directionWords'

const STORAGE_KEY = 'dwHinHerSetup'
...
const availableItems = computed(() => filterHinHerItems({ levels: levels.value }).length)
...
function start() {
  router.push({
    name: 'directionwords-hinher-run',
    query: {
...
<template>
  <div class="page setup-page">
    <header class="section-header">
      <div>
        <div class="breadcrumb">Modul · hin &amp; her · Grundregel</div>
        <h1 class="section-title">Hin or her?<em>.</em></h1>
        <p class="section-subtitle">
          A scene diagram shows where the speaker stands — pick the direction the
          sentence describes: <em>hin</em> (away from the speaker) or <em>her</em>
          (toward the speaker). Watch out for the occasional <em>hier</em> button —
          it names a place, not a movement, and it is never the answer.
        </p>
      </div>
    </header>
