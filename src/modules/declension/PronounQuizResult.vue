...
import type { PronounQuestion } from '../../composables/useDeclensionQuiz'
...
onMounted(() => {
  try {
    const raw = sessionStorage.getItem('gt:lastDeclPronoun')
...
const summary = computed(() => {
  if (pct.value >= 80) return 'Stark. The pronoun paradigm is locked in.'
  if (pct.value >= 50) return 'Solid. A few cases still want another pass.'
...
<template>
  <div v-if="!data" class="page">
    <div class="alert alert-info">
      ...
      Nothing to show — finish a pronoun-forms quiz to land here with your score.
    </div>
    ...
    <div class="decl-result-list">
      ...
      >
        ...
        <div class="decl-result-prompt">
          <div class="drp-german">{{ q.entry.nominative }}</div>
          <div v-if="q.entry.meta" class="drp-meta">{{ q.entry.meta }}</div>
        </div>
          ...
          >
            ...
            <span class="drl-value">
              <template v-if="row.skipped">
                <span class="vr-stamp vr-stamp-empty">—</span>
                <span class="drl-expected">
                  <span class="vr-stamp vr-stamp-empty">—</span>
                </span>
              </template>
              <template v-else>
