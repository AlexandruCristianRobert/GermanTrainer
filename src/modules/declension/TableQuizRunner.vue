...
import { useTableQuiz } from '../../composables/useDeclensionQuiz'
import { saveQuizRun } from '../../composables/useQuizHistory'
import {
  DECL_LEVELS, DECL_DETERMINERS, DECL_GENDERS,
  type DeclensionEntry, type DeclLevel, type Determiner, type DeclGender, type DeclCase
...
const { sampleDeclensionTables } = useDeclension()

const loading = ref(true)
const error = ref<string | null>(null)
let quiz: ReturnType<typeof useTableQuiz> | null = null
const ready = ref(false)
const inputs = ref<string[]>([])
const submitted = ref(false)
const startedAtMs = ref<number>(0)
const selectedLevels = ref<DeclLevel[]>([])
const selectedDeterminers = ref<Determiner[]>([])
const selectedGenders = ref<DeclGender[]>([])
...
onMounted(() => {
  ...
  const f = {
    levels: csv<DeclLevel>(route.query.levels, DECL_LEVELS),
    determiners: csv<Determiner>(route.query.determiners, DECL_DETERMINERS),
    genders: csv<DeclGender>(route.query.genders, DECL_GENDERS)
  }
...
watch(finished, (now) => {
  ...
  saveQuizRun({
    ...
    meta: {
      declLevels: selectedLevels.value,
      declDeterminers: selectedDeterminers.value
    }
  })
  try {
    sessionStorage.setItem('gt:lastDeclTable', JSON.stringify({
  ...
  router.push({ name: 'declension-table-result' })
})
</script>
  ...
  <div v-else-if="current" class="page">
    <div class="decl-quiz-stage">
      ...
      <div class="prompt-card decl-prompt">
        <div class="prompt-german decl-prompt-german">{{ current.entry.forms.nominative }}</div>
        <div class="decl-prompt-meta">{{ current.entry.gender }} · {{ current.entry.determiner }}</div>
      </div>
