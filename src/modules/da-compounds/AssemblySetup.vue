...
import { filterAssemblyItems, ASSEMBLY_PREPS } from '../../composables/useDaAssemblyQuiz'
import { COLLOCATION_LEVELS, type CollocationLevel } from '../../data/collocations'

const STORAGE_KEY = 'dacAssemblySetup'
const router = useRouter()

const levels = ref<CollocationLevel[]>(['B1', 'B2'])
const preps  = ref<string[]>([...ASSEMBLY_PREPS])
...
function load() {
  try {
    ...
    if (s.preps)  preps.value  = s.preps.filter(p => ASSEMBLY_PREPS.includes(p))
...
const availableItems = computed(() =>
  filterAssemblyItems({ levels: levels.value, preps: preps.value }).length
)
...
<template>
  <div class="page setup-page">
    <header class="section-header">
      <div>
        <div class="breadcrumb">Modul · Pronominaladverbien · Satzbau</div>
        <h1 class="section-title">Sentence assembly<em>.</em></h1>
        <p class="section-subtitle">
          A scrambled German sentence, tile by tile — tap them into the right order.
          Each tile is pre-inflected, so you never fight spelling: only word order counts.
          Some sentences accept a curated fronted variant too.
        </p>
      </div>
    </header>
    ...
    <div class="field">
      <div class="field-row">
        <div class="field-label">Preposition · {{ preps.length }} of {{ ASSEMBLY_PREPS.length }}</div>
        <div class="field-actions">
          <button class="btn btn-quiet" type="button" @click="preps = [...ASSEMBLY_PREPS]">All</button>
          <button class="btn btn-quiet" type="button" @click="preps = []">None</button>
        </div>
      </div>
      <div class="chip-row">
        <button
          v-for="p in ASSEMBLY_PREPS" :key="p"
