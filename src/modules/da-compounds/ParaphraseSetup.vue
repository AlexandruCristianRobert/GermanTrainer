...
import { filterParaphraseItems, PARAPHRASE_PREPS } from '../../composables/useDaParaphraseQuiz'
import { COLLOCATION_LEVELS, type CollocationLevel } from '../../data/collocations'

const STORAGE_KEY = 'dacParaphraseSetup'
const router = useRouter()

const levels = ref<CollocationLevel[]>(['B1', 'B2'])
const preps  = ref<string[]>([...PARAPHRASE_PREPS])
...
function load() {
  try {
    ...
    if (s.preps)  preps.value  = s.preps.filter(p => PARAPHRASE_PREPS.includes(p))
...
function save() {
  try {
    const payload: Stored = { levels: levels.value, preps: preps.value, preset: preset.value }
    localStorage.setItem(STORAGE_KEY, JSON.stringify(payload))
  } catch { /* ignore */ }
}

onMounted(load)
watch([levels, preps, preset], save, { deep: true })

const availableItems = computed(() =>
  filterParaphraseItems({ levels: levels.value, preps: preps.value }).length
)
...
function start() {
  router.push({
    name: 'dacompounds-paraphrase-run',
    query: {
...
<template>
  <div class="page setup-page">
    <header class="section-header">
      <div>
        <div class="breadcrumb">Modul · Pronominaladverbien · Paraphrase</div>
        <h1 class="section-title">Paraphrase<em>.</em></h1>
        <p class="section-subtitle">
          The same idea, two ways: a noun-phrase sentence needs the bare
          preposition (Ich kümmere mich <strong>um</strong> die Lieferung),
          its clause paraphrase needs the da-compound Korrelat (Ich kümmere
          mich <strong>darum</strong>, dass sie ankommt). Fill both gaps —
          the card only counts once both are right.
        </p>
      </div>
    </header>
    ...
    <div class="field">
      <div class="field-row">
        <div class="field-label">Preposition · {{ preps.length }} of {{ PARAPHRASE_PREPS.length }}</div>
        <div class="field-actions">
          <button class="btn btn-quiet" type="button" @click="preps = [...PARAPHRASE_PREPS]">All</button>
          <button class="btn btn-quiet" type="button" @click="preps = []">None</button>
        </div>
      </div>
      <div class="chip-row">
        <button
          v-for="p in PARAPHRASE_PREPS" :key="p"
