...
function load() {
  try {
    ...
    if (s.preps)  preps.value  = s.preps.filter(p => MATCH_PREPS.includes(p))
...
const availableItems = computed(() =>
  filterMatchCollocations({ levels: levels.value, roles: roles.value, preps: preps.value }).length
)
...
<template>
  <div class="page setup-page">
    <header class="section-header">
      <div>
        <div class="breadcrumb">Modul · Pronominaladverbien · Zuordnung</div>
        <h1 class="section-title">Matching<em>.</em></h1>
        <p class="section-subtitle">
          Tap a word on the left, then its da-compound on the right — pair every
          collocation on the screen before checking your answers. Each screen mixes
          different governing prepositions so no two right-side chips ever match.
        </p>
      </div>
    </header>
    ...
    <div class="field">
      <div class="field-row">
        <div class="field-label">Preposition · {{ preps.length }} of {{ MATCH_PREPS.length }}</div>
        <div class="field-actions">
          <button class="btn btn-quiet" type="button" @click="preps = [...MATCH_PREPS]">All</button>
          <button class="btn btn-quiet" type="button" @click="preps = []">None</button>
        </div>
      </div>
      <div class="chip-row">
        <button
          v-for="p in MATCH_PREPS" :key="p"
    ...
    <div class="field">
      <div class="field-label">Number of pairs</div>
    ...
    <div class="setup-actions">
      ...
      <button
        ...
        @click="start"
      >
        Start drill · {{ effectiveCount }} pairs <span aria-hidden="true">→</span>
      </button>
    </div>
  </div>
</template>
