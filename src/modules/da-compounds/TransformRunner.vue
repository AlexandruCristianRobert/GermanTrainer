...
import {
  useDaTransformQuiz, sampleTransformItems, splitTransformSentence,
  TRANSFORM_PREPS, type TransformMode,
...
type Quiz = ReturnType<typeof useDaTransformQuiz>
const quiz = shallowRef<Quiz | null>(null)
const startedAtMs = ref(0)
const historySaved = ref(false)
const modeUsed = ref<TransformMode>('pick')
...
onMounted(() => {
  ...
  const preps = csv<string>(route.query.preps, TRANSFORM_PREPS)
  const mode: TransformMode = route.query.mode === 'type' ? 'type' : 'pick'

  try {
    const items = sampleTransformItems(count, { levels, roles, preps })
    if (items.length === 0) {
      error.value = 'Nothing to drill — adjust your filters.'
    } else {
      modeUsed.value = mode
      quiz.value = useDaTransformQuiz(items, { mode })
...
const sentenceParts = computed(() => current.value ? splitTransformSentence(current.value.item) : null)
...
function retryWrong() {
  ...
  quiz.value = useDaTransformQuiz(shuffle(wrong), { mode: modeUsed.value })
  resetInputs()
...
function restart() {
  router.push({ name: 'dacompounds-transform' })
}
</script>
...
<template>
  ...
  <div v-else-if="finished && ready && !showRetryModal" class="page result-page">
    <header class="section-header">
      <div>
        <div class="breadcrumb">Auswertung · Sache oder Person</div>
        <div class="result-score">{{ score }} / {{ total }} correct</div>
        <p class="section-subtitle">Thing-or-person transform drill complete.</p>
      </div>
  ...
  <div v-else-if="current && ready" class="page">
    <div class="drill-stage" ref="cardRef" tabindex="-1">
      <div class="quiz-meta">
        ...
        <button class="btn btn-quiet" type="button" @click="router.push({ name: 'dacompounds-transform' })">End drill</button>
      </div>
      ...
      <!-- Prompt: the sentence with the replaceable object bolded -->
      <div class="drill-prompt">
        <p class="micro-mark drill-instruction">Ersetzen: Sache → da-Wort, Person → Präposition + Pronomen</p>
        <p class="drill-sentence">
          {{ sentenceParts!.pre }}<strong class="drill-object">{{ sentenceParts!.object }}</strong>{{ sentenceParts!.post }}
        </p>
      </div>
      ...
      <div v-else class="type-row">
        <input
          ...
          type="text"
          placeholder="da-Wort oder Präposition + Pronomen"
          :readonly="submitted"
          autocomplete="off"
          autocapitalize="off"
          spellcheck="false"
