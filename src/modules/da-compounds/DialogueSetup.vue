...
function load() {
  try {
    ...
    if (s.preps)  preps.value  = s.preps.filter(p => DIALOGUE_PREPS.includes(p))
...
const availableItems = computed(() =>
  filterDialogueItems({ levels: levels.value, roles: roles.value, preps: preps.value }).length
)
...
<template>
  <div class="page setup-page">
    <header class="section-header">
      <div>
        <div class="breadcrumb">Modul · Pronominaladverbien · Dialog</div>
        <h1 class="section-title">Dialogue<em>.</em></h1>
        <p class="section-subtitle">
          The full pairing: a wo-question opens (Worauf wartest du?) and a natural
          reply answers it with the da-compound as Korrelat (Ich warte darauf, dass …).
          Fill both gaps — the card only counts once both are right.
        </p>
      </div>
    </header>
    ...
    <div class="field">
      <div class="field-row">
        <div class="field-label">Preposition · {{ preps.length }} of {{ DIALOGUE_PREPS.length }}</div>
        <div class="field-actions">
          <button class="btn btn-quiet" type="button" @click="preps = [...DIALOGUE_PREPS]">All</button>
          <button class="btn btn-quiet" type="button" @click="preps = []">None</button>
        </div>
      </div>
      <div class="chip-row">
        <button
          v-for="p in DIALOGUE_PREPS" :key="p"
