...
function load() {
  try {
    ...
    if (s.preps)  preps.value  = s.preps.filter(p => WO_QUESTION_PREPS.includes(p))
...
function save() {
  try {
    const payload: Stored = { levels: levels.value, roles: roles.value, preps: preps.value, preset: preset.value }
...
const availableItems = computed(() =>
  filterWoQuestionItems({ levels: levels.value, roles: roles.value, preps: preps.value }).length
)
...
function start() {
  router.push({
    name: 'dacompounds-wo-question-run',
    query: {
...
<template>
  <div class="page setup-page">
    <header class="section-header">
      <div>
        <div class="breadcrumb">Modul · Pronominaladverbien · W-Fragen</div>
        <h1 class="section-title">Wo-questions<em>.</em></h1>
        <p class="section-subtitle">
          A statement uses a prepositional collocation — its object is NOT flagged for
          you. Decide for yourself: a THING is asked about with a wo-compound
          (Worauf, Woran …); a PERSON is asked about with Präposition + wen/wem
          (Auf wen, Mit wem …) — never a wo-compound.
        </p>
      </div>
    </header>
    ...
    <div class="field">
      <div class="field-row">
        <div class="field-label">Preposition · {{ preps.length }} of {{ WO_QUESTION_PREPS.length }}</div>
        <div class="field-actions">
          <button class="btn btn-quiet" type="button" @click="preps = [...WO_QUESTION_PREPS]">All</button>
          <button class="btn btn-quiet" type="button" @click="preps = []">None</button>
        </div>
      </div>
      <div class="chip-row">
        <button
          v-for="p in WO_QUESTION_PREPS" :key="p"
