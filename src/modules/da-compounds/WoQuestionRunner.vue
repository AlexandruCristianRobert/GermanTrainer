  ...
  <div v-else-if="current && ready" class="page">
    <div class="sub-stage" ref="cardRef" tabindex="-1">
      ...
      <div class="sub-prompt">
        <p class="micro-mark sub-instruction">Sache oder Person? Frag danach.</p>
        <p class="sub-statement">{{ current.item.statement }}</p>
        <p class="sub-stem">
          <span class="sub-gap" :class="{ filled: submitted, ok: submitted && current.isCorrect, err: submitted && !current.isCorrect }">{{ submitted ? current.answer : '＿＿＿' }}</span> {{ scaffoldPost }}
        </p>
      </div>
      ...
      <div v-if="submitted" class="sub-feedback">
        ...
        <template v-else>
          ...
          <div class="sub-reveal" :style="prepColorStyle(current.colloc.preposition)">
            ...
            <div class="sub-reveal-rule micro-mark">Sache → wo(r)-, Person → Präposition + wen/wem</div>
