...
import {
  filterSubstitutionItems, SUBSTITUTION_PREPS, type SubstitutionMode,
...
const STORAGE_KEY = 'dacSubstitutionSetup'
...
const mode   = ref<SubstitutionMode>('pick')
...
function start() {
  router.push({
    name: 'dacompounds-substitution-run',
    query: {
  ...
  <div class="page setup-page">
    <header class="section-header">
      <div>
        ...
        <h1 class="section-title">Gap-fill<em>.</em></h1>
        <p class="section-subtitle">
          A natural sentence uses a fixed-preposition collocation; a follow-up sentence
          replaces that phrase with a da-compound and leaves you the gap. Pick the compound
          from four options, or type it yourself.
        </p>
      </div>
    </header>
    ...
    <div class="field">
      ...
      <p class="micro-mark grading-hint">
        {{ mode === 'pick'
          ? 'Pick the correct da-compound from four options.'
          : 'Type the da-compound yourself — folded umlauts are accepted (darueber = darüber).' }}
      </p>
    </div>
