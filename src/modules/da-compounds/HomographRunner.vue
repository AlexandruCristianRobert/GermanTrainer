...
import {
  useDaHomographQuiz, sampleHomographItems, splitHomographSentence,
  type HomographQuestion, type HomographReading,
} from '../../composables/useDaHomographQuiz'
...
type Quiz = ReturnType<typeof useDaHomographQuiz>
...
onMounted(() => {
  ...
  try {
    const items = sampleHomographItems(count, { levels })
    if (items.length === 0) {
      error.value = 'Nothing to drill — adjust your filters.'
    } else {
      quiz.value = useDaHomographQuiz(items)
...
const sentenceParts = computed(() =>
  current.value ? splitHomographSentence(current.value.item.sentence, current.value.item.word) : null
)

/** The option label for a given reading on a question (for the result list / reveal). */
function labelFor(q: HomographQuestion, reading: HomographReading | null): string {
  if (!reading) return ''
  return q.options.find(o => o.reading === reading)?.label ?? ''
}

// ── actions ──────────────────────────────────────────────────────────────────
function pick(reading: HomographReading) {
...
function retryWrong() {
  ...
  quiz.value = useDaHomographQuiz(shuffle(wrong))
  resetCard()
...
function restart() {
  router.push({ name: 'dacompounds-homograph' })
}
</script>
...
<template>
  ...
  <div v-else-if="finished && ready && !showRetryModal" class="page result-page">
    <header class="section-header">
      <div>
        <div class="breadcrumb">Auswertung · Homographs</div>
        <div class="result-score">{{ score }} / {{ total }} correct</div>
        <p class="section-subtitle">Homograph drill complete.</p>
      </div>
    ...
    <div class="sub-stage" ref="cardRef" tabindex="-1">
      ...
      <div v-if="submitted" class="sub-feedback">
        ...
        <div class="sub-reveal">
          ...
          >
            <span class="contrast-sense-prep">{{ opt.reading === 'compound' ? 'Pronominaladverb' : 'Konnektor/Adverb' }}</span>
