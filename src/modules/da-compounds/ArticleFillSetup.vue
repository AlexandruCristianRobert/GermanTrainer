...
import { filterArticleItems, ARTICLE_PREPS } from '../../composables/useDaArticleQuiz'
import { COLLOCATION_LEVELS, type CollocationLevel } from '../../data/collocations'

const STORAGE_KEY = 'dacArticleSetup'
const router = useRouter()

const levels = ref<CollocationLevel[]>(['B1', 'B2'])
const preps  = ref<string[]>([...ARTICLE_PREPS])
...
function load() {
  try {
    ...
    if (s.preps)  preps.value  = s.preps.filter(p => ARTICLE_PREPS.includes(p))
...
const availableItems = computed(() =>
  filterArticleItems({ levels: levels.value, preps: preps.value }).length
)
...
<template>
  <div class="page setup-page">
    <header class="section-header">
      <div>
        <div class="breadcrumb">Modul · Pronominaladverbien · Artikel</div>
        <h1 class="section-title">Article fill<em>.</em></h1>
        <p class="section-subtitle">
          Two-way prepositions in verb objects mostly take Akkusativ — <em>an ein___ Projekt</em>
          → <em>einen</em> — except a handful of Dativ verbs like <em>arbeiten an</em>. Type the
          full article word; its case comes from the collocation, not from guessing.
        </p>
      </div>
    </header>
    ...
    <div class="field">
      <div class="field-row">
        <div class="field-label">Preposition · {{ preps.length }} of {{ ARTICLE_PREPS.length }}</div>
        <div class="field-actions">
          <button class="btn btn-quiet" type="button" @click="preps = [...ARTICLE_PREPS]">All</button>
          <button class="btn btn-quiet" type="button" @click="preps = []">None</button>
        </div>
      </div>
      <div class="chip-row">
        <button
          v-for="p in ARTICLE_PREPS" :key="p"
