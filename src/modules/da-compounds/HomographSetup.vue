...
import { filterHomographItems } from '../../composables/useDaHomographQuiz'
import { COLLOCATION_LEVELS, type CollocationLevel } from '../../data/collocations'

const STORAGE_KEY = 'dacHomographSetup'
...
const availableItems = computed(() =>
  filterHomographItems({ levels: levels.value }).length
)
...
function start() {
  router.push({
    name: 'dacompounds-homograph-run',
    query: {
...
<template>
  <div class="page setup-page">
    <header class="section-header">
      <div>
        <div class="breadcrumb">Modul · Pronominaladverbien · Homographs</div>
        <h1 class="section-title">Homographs<em>.</em></h1>
        <p class="section-subtitle">
          The same word, two readings: <strong>damit</strong> the purpose
          conjunction ("so that") vs. <strong>damit</strong> = mit + Sache
          ("with it"). The sentence forces exactly one — pick the reading it
          demands.
        </p>
      </div>
    </header>
