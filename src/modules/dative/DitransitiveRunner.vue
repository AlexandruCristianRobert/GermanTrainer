...
import {
  useDativeQuiz, buildDitransitiveCards, filterDitransitiveItems, type DativeQuizCard,
...
// Rule-driven family (spec, family VI): ditransitives are NOT in gt:dativeLedger —
// their dative is predictable from the recipient role, not memorised. This runner
// deliberately imports no useDativeLedger / bumpDativeLedger. Band-tracked only,
// via saveQuizRun below.
...
const items = ref<DitransitiveItem[]>([])
const queriedLevels = ref<DativeDrillLevel[]>([])
const queriedRoles = ref<string[]>([])
...
onMounted(() => {
  ...
  const roles = csv<GapRole>(route.query.roles, ROLES)

  try {
    const sampled = shuffle(filterDitransitiveItems({ levels, roles }), count)
    if (sampled.length === 0) {
      error.value = 'Nothing to drill — adjust your filters.'
    } else {
      items.value = sampled
      queriedLevels.value = levels
      queriedRoles.value = roles
      quiz.value = useDativeQuiz(buildDitransitiveCards(sampled))
...
// The prompt sentence carries a single ___ on the gapped object's article —
...
// Record the main round once. NO ledger bump anywhere in this function — T7 is
...
function retryWrong() {
  ...
  quiz.value = useDativeQuiz(buildDitransitiveCards(resampled))
  resetInputs()
...
function restart() {
  router.push({ name: 'dative-ditransitive' })
}
...
<template>
  ...
  <div v-else-if="error" class="page">
    ...
    <button class="btn btn-ghost" @click="router.push({ name: 'dative-ditransitive' })">← Back to setup</button>
  </div>
  ...
  <div v-else-if="finished && ready && !showRetryModal" class="page result-page">
    <header class="section-header">
      <div>
        <div class="breadcrumb">Auswertung · Welches Objekt?</div>
        <div class="result-score">{{ score }} / {{ total }} correct</div>
        <p class="section-subtitle">Ditransitive round complete.</p>
      </div>
  ...
  <div v-else-if="current && ready" class="page">
    <div class="drill-stage" ref="cardRef" tabindex="-1">
      <div class="quiz-meta">
        ...
        <button class="btn btn-quiet" type="button" @click="router.push({ name: 'dative-ditransitive' })">End drill</button>
      </div>
      ...
      <!-- Pick mode: 2 option buttons — the two candidate articles for the gapped object -->
      ...
      <!-- Feedback always reveals the filled sentence, translation, and the note —
           which names whether the gap was the recipient (dative) or the thing
           (accusative) — win or lose. -->
