...
import {
  useDativeQuiz, buildReflexiveCards, filterReflexiveItems, type DativeQuizCard,
...
import type { ReflexiveItem } from '../../data/dativeConsequences'
import RetryModal from '../../components/RetryModal.vue'

// Rule-driven family (spec, family X): the reflexive dative/accusative split
// is predictable from whether an accusative object is already present — it
// is not a ledger item, and the verbs drilled here (waschen, vorstellen,
// anziehen, kämmen, rasieren, …) are ordinary transitives, not DATIVE_VERBS
// members. This runner deliberately imports no useDativeLedger /
// bumpDativeLedger. Band-tracked only, via saveQuizRun below.
...
onMounted(() => {
  ...
  try {
    const sampled = shuffle(filterReflexiveItems({ levels }), count)
    if (sampled.length === 0) {
      error.value = 'Nothing to drill — adjust your filters.'
    } else {
      items.value = sampled
      queriedLevels.value = levels
      quiz.value = useDativeQuiz(buildReflexiveCards(sampled))
...
// Every T13 prompt carries a single ___ gap for the reflexive pronoun — split
// it so the gap can be styled independently, and always fill it in feedback.
...
// Record the main round once. NO ledger bump anywhere in this function — T13
...
function retryWrong() {
  ...
  quiz.value = useDativeQuiz(buildReflexiveCards(resampled))
  resetInputs()
...
function restart() {
  router.push({ name: 'dative-reflexive' })
}
...
<template>
  ...
  <div v-else-if="error" class="page">
    ...
    <button class="btn btn-ghost" @click="router.push({ name: 'dative-reflexive' })">← Back to setup</button>
  </div>
  ...
  <div v-else-if="finished && ready && !showRetryModal" class="page result-page">
    <header class="section-header">
      <div>
        <div class="breadcrumb">Auswertung · Reflexiver Dativ</div>
        <div class="result-score">{{ score }} / {{ total }} correct</div>
        <p class="section-subtitle">Reflexive-dative round complete.</p>
      </div>
  ...
  <div v-else-if="current && ready" class="page">
    <div class="drill-stage" ref="cardRef" tabindex="-1">
      <div class="quiz-meta">
        ...
        <button class="btn btn-quiet" type="button" @click="router.push({ name: 'dative-reflexive' })">End drill</button>
      </div>
      ...
      <!-- Pick mode: 2 option buttons — the person-matched mir/mich or dir/dich pair -->
      ...
      <!-- Feedback always reveals the filled sentence, translation, and the note —
           which names whether an accusative object was already present (dative
           reflexive) or not (accusative reflexive) — win or lose. -->
