...
import {
  useDativeQuiz, buildObjectOrderCards, filterObjectOrderItems, type DativeQuizCard,
...
import type { ObjectOrderItem } from '../../data/dativeDitransitive'
import RetryModal from '../../components/RetryModal.vue'

// Rule-driven family (spec, family VI): object order is NOT in gt:dativeLedger —
// the correct order is derived mechanically by objectOrderAnswer(), not memorised
// per verb. This runner deliberately imports no useDativeLedger / bumpDativeLedger.
// Band-tracked only, via saveQuizRun below.

type OrderKind = ObjectOrderItem['kind']
const KINDS: OrderKind[] = ['nn', 'pp', 'mixed']
...
const items = ref<ObjectOrderItem[]>([])
...
onMounted(() => {
  ...
  const kinds = csv<OrderKind>(route.query.kinds, KINDS)

  try {
    const sampled = shuffle(filterObjectOrderItems({ levels, kinds }), count)
    if (sampled.length === 0) {
      ...
      quiz.value = useDativeQuiz(buildObjectOrderCards(sampled))
...
// The prompt carries a single ___ where both objects go, as one continuation —
// split it so the gap can be styled independently.
...
// Record the main round once. NO ledger bump anywhere in this function — T8 is
// rule-driven (spec): band-tracked only via saveQuizRun. Retry rounds and
...
function retryWrong() {
  ...
  quiz.value = useDativeQuiz(buildObjectOrderCards(resampled))
  resetInputs()
...
function restart() {
  router.push({ name: 'dative-object-order' })
}
...
<template>
  ...
  <div v-else-if="error" class="page">
    ...
    <button class="btn btn-ghost" @click="router.push({ name: 'dative-object-order' })">← Back to setup</button>
  </div>
  ...
  <div v-else-if="finished && ready && !showRetryModal" class="page result-page">
    <header class="section-header">
      <div>
        <div class="breadcrumb">Auswertung · Objektfolge</div>
        <div class="result-score">{{ score }} / {{ total }} correct</div>
        <p class="section-subtitle">Object-order round complete.</p>
      </div>
  ...
  <div v-else-if="current && ready" class="page">
    <div class="drill-stage" ref="cardRef" tabindex="-1">
      <div class="quiz-meta">
        ...
        <button class="btn btn-quiet" type="button" @click="router.push({ name: 'dative-object-order' })">End drill</button>
      </div>
      ...
      <p class="micro-mark oo-hint">Wie geht der Satz neutral weiter?</p>

      <!-- Pick mode: 2 option buttons — the rule-derived order and its flip -->
      ...
      <!-- Feedback always reveals the full correct sentence, translation, and the
           note — for two-noun items the flip is merely marked rather than flatly
           ungrammatical, and the note text says so — win or lose. -->
