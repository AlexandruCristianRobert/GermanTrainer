...
import { filterReflexiveItems } from '../../composables/useDativeDrill'
import { DATIVE_DRILL_LEVELS, type DativeDrillLevel } from '../../data/dativeExperiencer'

// T13 Reflexiver Dativ (family X): deliberately NO dative/accusative kind
// filter here — REFLEXIVE_ITEMS pairs both kinds on the same five contrast
// verbs (REFLEXIVE_CONTRAST_VERBS), and letting the learner narrow to only
// one kind would make every answer mir/dir (or mich/dich) — one-button-
// winnable, the Direction Words T8 mistake. Levels + count only.

const STORAGE_KEY = 'datReflexiveSetup'
...
const availableItems = computed(() =>
  filterReflexiveItems({ levels: levels.value }).length
)
...
<template>
  <div class="page setup-page">
    <header class="section-header">
      <div>
        <div class="breadcrumb">Kapitel XIII · Dativ · Reflexiver Dativ</div>
        <h1 class="section-title">Reflexiver Dativ<em>.</em></h1>
        <p class="section-subtitle">
          <em>ich wasche mich</em> — but <em>ich wasche mir die Hände</em>: the moment an
          accusative object is already in the sentence, the reflexive moves to the dative.
          mir/dir against mich/dich, and the sentence decides.
        </p>
      </div>
    </header>
