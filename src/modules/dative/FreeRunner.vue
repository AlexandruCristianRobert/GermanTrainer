...
import {
  useDativeQuiz, buildFreeCards, filterFreeItems, type DativeQuizCard,
...
import type { FreeDativeItem } from '../../data/dativeFree'
import RetryModal from '../../components/RetryModal.vue'

// Rule-driven family (spec, family VIII): free datives are NOT in gt:dativeLedger —
// they're optional add-ons the verb never asked for, not memorised verb government.
// This runner deliberately imports no useDativeLedger / bumpDativeLedger. Band-tracked
// only, via saveQuizRun below (buildFreeCards already sets every card's ledgerKey to null).

const FREE_KINDS = ['drop', 'classify'] as const
...
const items = ref<FreeDativeItem[]>([])
...
onMounted(() => {
  ...
  const kinds = csv<string>(route.query.kinds, FREE_KINDS)

  try {
    const sampled = shuffle(filterFreeItems({ levels, kinds }), count)
    if (sampled.length === 0) {
      ...
      quiz.value = useDativeQuiz(buildFreeCards(sampled))
...
// Keyboard: 1–N to pick a choice — drop cards carry two options (weglassbar /
// obligatorisch), classify cards carry three (the three readings); Enter advances.
...
// Record the main round once. NO ledger bump anywhere in this function — T10 is
// rule-driven (spec): free datives are not ledger items, band-tracked only via
// saveQuizRun. Retry rounds and empty rounds never record (ADR-0010).
function recordRun() {
...
function retryWrong() {
  ...
  quiz.value = useDativeQuiz(buildFreeCards(resampled))
  resetInputs()
...
<template>
  ...
  <div v-else-if="error" class="page">
    ...
    <button class="btn btn-ghost" @click="router.push({ name: 'dative-free' })">← Back to setup</button>
  </div>
  ...
  <div v-else-if="finished && ready && !showRetryModal" class="page result-page">
    <header class="section-header">
      <div>
        <div class="breadcrumb">Auswertung · Freier Dativ</div>
        <div class="result-score">{{ score }} / {{ total }} correct</div>
        <p class="section-subtitle">Free-dative round complete.</p>
      </div>
  ...
  <div v-else-if="current && ready" class="page">
    <div class="drill-stage" ref="cardRef" tabindex="-1">
      <div class="quiz-meta">
        ...
        <button class="btn btn-quiet" type="button" @click="router.push({ name: 'dative-free' })">End drill</button>
      </div>
      ...
      <!-- Pick mode: two buttons for drop cards (weglassbar / obligatorisch),
           three for classify cards (the three free-dative readings). -->
      ...
      <!-- Feedback always reveals translation and the explanation — the note carries
           BOTH the drop test on the free sentence AND the dative-verb counterexample
           that cannot drop its object, win or lose. -->
