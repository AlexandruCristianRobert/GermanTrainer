...
import { filterSubjectItems } from '../../composables/useDativeDrill'
import { DATIVE_DRILL_LEVELS, type DativeDrillLevel } from '../../data/dativeExperiencer'

const STORAGE_KEY = 'datSubjectSetup'
const router = useRouter()

type SubjectKind = 'subject' | 'agreement'
const KINDS: SubjectKind[] = ['subject', 'agreement']
const KIND_LABELS: Record<SubjectKind, string> = {
  subject: 'Subjekt finden',
  agreement: 'Verbform wählen',
}
...
const availableItems = computed(() =>
  filterSubjectItems({ levels: levels.value, kinds: kinds.value }).length
)
...
<template>
  <div class="page setup-page">
    <header class="section-header">
      <div>
        <div class="breadcrumb">Kapitel XIII · Dativ · Wer ist Subjekt?</div>
        <h1 class="section-title">Wer ist Subjekt?<em>.</em></h1>
        <p class="section-subtitle">
          Die Schuhe gefallen mir — the thing is the subject and controls the verb; the person is
          dative. This drill exists to kill *Ich gefalle das Buch and *Die Schuhe gefällt mir.
        </p>
      </div>
    </header>
