...
// The six determinate cases — 'varies' is intentionally excluded.
const CASE_GOVERNMENT_CASES = [
  'accusative',
  'dative',
  'dative+accusative',
  'genitive',
...
type CaseGovernmentCase = (typeof CASE_GOVERNMENT_CASES)[number]

const CASE_LABELS: Record<CaseGovernmentCase, string> = {
  'accusative':       'Akkusativ',
  'dative':           'Dativ',
  'dative+accusative':'Dativ + Akkusativ',
  'genitive':         'Genitiv',
  'reflexive':        'Reflexiv',
  'none':             'Kein Objekt',
}

const STORAGE_KEY = 'gtCaseGovernment'
...
function save() {
  try {
    const payload: Stored = {
      levels: levels.value, types: types.value, cases: cases.value,
...
onMounted(load)
watch([levels, types, cases, preset, customCount], save, { deep: true })

// Filter excludes 'varies' automatically since it's not in CASE_GOVERNMENT_CASES
const availableVerbs = computed(() =>
  filter({ levels: levels.value, types: types.value, cases: cases.value }).length
)
...
function start() {
  router.push({
    name: 'verbs-case-government-run',
    query: {
...
<template>
  <div class="page setup-page">
    <header class="section-header">
      <div>
        <div class="breadcrumb">Kapitel III · Verben · Rektion</div>
        <h1 class="section-title">Case government<em>.</em></h1>
        <p class="section-subtitle">
          Filter the verb pool and pick how many verbs to drill.
          For each verb, identify the grammatical case it governs.
        </p>
      </div>
    </header>
    ...
    <div class="field">
      <div class="field-row">
        <div class="field-label">Case filter · {{ cases.length }} of {{ CASE_GOVERNMENT_CASES.length }}</div>
        <div class="field-actions">
          <button class="btn btn-quiet" type="button" @click="cases = [...CASE_GOVERNMENT_CASES]">All</button>
          <button class="btn btn-quiet" type="button" @click="cases = []">None</button>
        </div>
      </div>
      <div class="chip-row">
        <button
          v-for="c in CASE_GOVERNMENT_CASES" :key="c"
