...
//
// Spoken Teil 2 — setup. A separate test over the same Discussion concept
// (CONTEXT.md → "Modality"): same Topic pool, same turn targets, same partner
// stance. What differs is the microphone gate and the preparation minute.
...
// VoiceRunStash lives in ../../data/sprechen so prep and runner can import it.
...
onMounted(async () => {
  ...
  try {
    ...
    if (typeof s.turnTarget === 'number' && (TURN_TARGETS as readonly number[]).includes(s.turnTarget)) {
      turnTarget.value = s.turnTarget as TurnTarget
    }
...
watch([turnTarget, stance, prepSeconds], () => {
  try {
    const prev = JSON.parse(localStorage.getItem(STORAGE_KEY) ?? '{}') as Record<string, unknown>
    localStorage.setItem(STORAGE_KEY, JSON.stringify({
      ...prev, turnTarget: turnTarget.value, stance: stance.value, prepSeconds: prepSeconds.value
    }))
  } catch { /* ignore */ }
})
...
function start() {
  ...
  if (!canUseAi.value) {
    toast.error(
      ...
      { description: 'Set your API key in Settings before using AI.' }
    )
    return
  }
  const resolved: PartnerStance = stance.value === 'random'
    ? (Math.random() < 0.5 ? 'pro' : 'contra')
    : stance.value
  const stash: VoiceRunStash = {
  ...
  sessionStorage.setItem(VOICE_RUN_STASH_KEY, JSON.stringify(stash))
  router.push({ name: prepSeconds.value > 0 ? 'sprechen-voice-prep' : 'sprechen-voice-run' })
}
...
<template>
  <div class="page voice-setup">
    <header class="section-header">
      <div>
        <div class="breadcrumb">Sprechen Teil 2 · gesprochen · Etappe 01</div>
        <h1 class="section-title">Themenwahl<em>.</em></h1>
        <p class="section-subtitle">
          Dieselbe Diskussion wie im getippten Test — nur sprichst du sie.
          Der Partner liest seine Beiträge vor, du antwortest ins Mikrofon.
        </p>
      </div>
    </header>

    <div v-if="!micSupported" class="alert alert-warning">
      <span class="alert-label">Mikrofon nicht verfügbar</span>
      Dieser Browser kennt keine Spracherkennung. Chrome, Edge oder Safari ab 14.1
      funktionieren; Firefox liefert sie nur hinter einem about:config-Schalter.
      Der <router-link :to="{ name: 'sprechen-teil2' }">getippte Test</router-link>
      läuft überall.
    </div>
    ...
    <div class="setup-actions">
      ...
      <button class="btn btn-accent btn-meta" type="button"
        :disabled="!topic || !micSupported || !canUseAi" @click="start">
