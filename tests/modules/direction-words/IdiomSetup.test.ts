...
import IdiomSetup from '../../../src/modules/direction-words/IdiomSetup.vue'
import { filterIdiomItems } from '../../../src/composables/useDwIdiomQuiz'
...
async function mountSetup() {
  ...
  await router.push({ name: 'directionwords-idioms' })
  const wrapper = mount(IdiomSetup, { global: { plugins: [router] } })
  await flushPromises()
  return { wrapper, router }
}

const DEFAULT_AVAILABLE = filterIdiomItems({ levels: ['B1', 'B2'] }).length
...
describe('IdiomSetup', () => {
  ...
  it('starts the run with the selected levels and the capped count', async () => {
    ...
    await flushPromises()
    expect(router.currentRoute.value.name).toBe('directionwords-idioms-run')
  ...
  it('persists the chips and the count preset under dwIdiomSetup', async () => {
    const { wrapper } = await mountSetup()
    const c1 = wrapper.findAll('.chip').find(c => c.text() === 'C1')!
    await c1.trigger('click')
    await wrapper.findAll('.segmented button').find(b => b.text() === '15')!.trigger('click')
    await flushPromises()
    expect(JSON.parse(localStorage.getItem('dwIdiomSetup')!)).toEqual({
      levels: ['B1', 'B2', 'C1'], preset: 15,
    })
    wrapper.unmount()
  })
  ...
  it('warns and disables Start when only A2 is selected (the bank has no A2 items)', async () => {
    ...
    expect(filterIdiomItems({ levels: ['A2'] })).toEqual([])
