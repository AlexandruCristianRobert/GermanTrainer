...
describe('buildSprechenGraderPrompt', () => {
  ...
  it('typed system prompt keeps the typed-modality wording and carries no spoken-only spelling caveat', () => {
  ...
  // 2026-08-10 grader recalibration (Task 2) — same three changes as Task 1's
  // Vortrag grader: a less severe persona, a typed-only Tippfehler rule that
  // lists spelling slips as feedback without letting them cost a criterion
  // score, and a KALIBRIERUNG paragraph naming 90–100 as the target band for
  // a strong-but-imperfect Diskussion.
  it('both personas say "faire, realistisch kalibrierte" instead of "strenge, kalibrierte"', () => {
    const typed = buildSprechenGraderPrompt(disc()).system
    const spoken = buildSprechenGraderPrompt(spokenDisc()).system
    expect(typed).toContain('faire, realistisch kalibrierte')
    expect(spoken).toContain('faire, realistisch kalibrierte')
    expect(typed).not.toContain('strenge, kalibrierte')
    expect(spoken).not.toContain('strenge, kalibrierte')
  })

  it('typed system prompt carries the Tippfehler rule and the KALIBRIERUNG paragraph; spoken does not carry the Tippfehler rule', () => {
    const typed = buildSprechenGraderPrompt(disc()).system
    const spoken = buildSprechenGraderPrompt(spokenDisc()).system
    expect(typed).toContain('Tippfehler (Kategorie "spelling")')
    expect(typed).toContain('KALIBRIERUNG')
    expect(typed).toContain('90–100')
    expect(spoken).not.toContain('Tippfehler (Kategorie "spelling")')
    expect(spoken).toContain('KALIBRIERUNG')     // calibration applies to both modalities
  })
